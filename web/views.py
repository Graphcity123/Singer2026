...
def analysis_view(request):
    ...
    # 从上到下渐变：顶深蓝 → 底浅蓝
    def add_lightness(data, n):
        return [(d[0], d[1], round(25 + i / max(n - 1, 1) * 45)) for i, d in enumerate(data)]

    match_sw = add_lightness(match_sw, len(match_sw))
    match_sm = add_lightness(match_sm, len(match_sm))
    match_wm = add_lightness(match_wm, len(match_wm))
    lyrics_singer = [(d[0], d[1], d[2], round(25 + i / max(len(lyrics_singer) - 1, 1) * 45)) for i, d in enumerate(lyrics_singer)]
    lyrics_writer = [(d[0], d[1], d[2], round(25 + i / max(len(lyrics_writer) - 1, 1) * 45)) for i, d in enumerate(lyrics_writer)]
